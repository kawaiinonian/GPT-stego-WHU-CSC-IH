...
def do_generate(hidden_bits, bit_index, length, n_ctx, tokenizer, model, device, context, temperature):
...
def generater(title: str, article_num: int, hidden_bits: str, length: int, batch_num:int, temperature:float):
...
def extract(text, length, title, temperature):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    bit_index = 0
    tokenizer = tokenization_bert.BertTokenizer(vocab_file='cache/vocab.txt')
    model = GPT2LMHeadModel.from_pretrained('model/')
...
# text = generater(title, 20, stream_b)
# print(text)
